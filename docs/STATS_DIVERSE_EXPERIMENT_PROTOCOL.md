...
- 新 train：18 類各 40，合計 720；只送這 720 題給教師。
- 歷史 train replay：18 類各 40，合計 720；完整 rows 物化於獨立 replay artifact，不送教師。
- development：18 類各 10，合計 180；只供 checkpoint 選擇。
- final blind：18 類各 10，合計 180；教師、訓練與 checkpoint 選擇均不可讀。
...
新 train 依 story 對照交錯，不以整類連續區塊餵入；所有 contrast group 必須完整。720 筆新教材再與 720 筆歷史 train replay 嚴格一新一舊交錯，形成唯一 1,440-row 訓練序列。runner 必須保存 ordered ID/role hash 與逐步 replay exposure；legacy development/final 永不作 replay。seed 31415 只做 CPU generator/order invariant fuzz，絕不訓練第二個學生，且本輪不宣稱已證明 training-seed robustness。
...
- 720 個新 train 問題以每 packet 12 題請求，首輪最多 60 calls；720 筆 replay 沿用已驗證 target，不呼叫教師。
- 只重試首輪未通過的項目一次；retry 只提供通用數學 rule card，不提供該題 oracle、reference expression、answer 或 final/development 內容。
- 全部 calls 上限 120；client-reported 累積成本硬上限 US$0.30。完成回覆缺 cost 欄時立即停止後續呼叫。
...
- 每個新 training target（無論 provenance）與每個 replay target 都必須同時通過 prompt→semantics、executable、symbolic dependency、數學結構與 `strict_one_line_expression=true`。這是 SFT 教材衛生，不是把學生評估改成語文能力；不合格式的教師 raw 必須走明示的 `canonical_corrected`，不得原樣進訓練。720/720 merged new targets、720/720 replay targets 未全數通過，或任何 pending／identity／receipt／interleave 不符時，training release 必須為 false。
...
11. 720 個新 training target 都能追溯至教師 raw 與明確 provenance；另 720 個 replay target 必須追溯至凍結歷史 train row。runner 實際訓練 teacher-results 內經驗證的 merged new targets與 materialized replay，不可只做事後 retention 評分；五道 gate 全過，pending=0。
...
- seed/data seed：2027。
- 一個 epoch，1,440 rows（720 new＋720 replay），batch 1、gradient accumulation 8，共 180 optimizer updates。
- learning rate：`1e-5`，constant schedule，無第二個 seed、第二輪、模型混合或補跑。
- 只在 60、120、180 步保存輕量 LoRA adapter；不保存 optimizer checkpoint。選點完成後只封存選中的一份。
...
候選 checkpoint 必須同時滿足；數學 gate 只能使用 `math_correct is True and executable is True`，不得引用 strict 格式欄位：

- 180 題 development 的 18 類各不低於同場 v15。
- v15 原本答對的 development 題 paired losses 為 0。
- development 總分至少比 v15 增加 10/180。
...
按 60→120→180 的固定順序，取**最早通過全部 gate**者；不在三點間追逐最高分。final 解封前，所有不需 final rows 就能判定的最終條件（legacy retention，以及若有 exposed regression 的最終門檻）亦必須先通過；不能明知某項最終條件失敗仍花費 final 推論。沒有合格 checkpoint 時停止，不解封 final，不追加訓練。

## 7. 事前成功門檻

選中 checkpoint 後，原 v15 與學生才以相同 prompt、greedy decoding、token budget 和 grader 同場跑 final blind。成功必須全部成立：

1. final blind 總分至少比 v15 增加 **15/180**。
