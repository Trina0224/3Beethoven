...
## Pre-training content audit amendment

After generation and before student training, the assistant read all 204 accepted explanations and misconceptions against the mathematical references. Twenty-seven records were flagged for false claims or misleading wording, including valid Poisson/expectation identities called mistakes, a weighted-sum arithmetic error, uniform-CDF proportionality, independence/overlap, and interval-width wording. These flags are not 27 independently measured teacher answer errors.

Run `review_stats_v0_5.py` for up to two targeted Llama revision/review pairs per flagged record, within the unchanged global 600-call ledger. This supplements the initial three-attempt generation rule; it does not change frozen questions, splits, reference answers, teacher model, student recipe, or tests. Preserve the pre-audit corpus under `before_content_review/`, original raw calls, new raw revision calls, explicit feedback, and exact target cache tags. Independently read revised prose before training. The assistant supplies error feedback, not replacement student explanation targets. This audit improves quality but is not a formal proof of every target.
