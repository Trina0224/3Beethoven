...
Numeric scoring is frozen before student inference at absolute tolerance 1e-10 or relative tolerance 1e-4, whichever is larger. The smaller absolute floor avoids falsely accepting badly approximated tiny binomial probabilities. Exact fractions are supported; no arbitrary numbers are extracted from prose. This changes the absolute floor used for the larger-valued v0.7 diagnostics and is not a post-result adjustment.
