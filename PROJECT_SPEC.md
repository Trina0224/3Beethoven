...
v0.14 student response distillation is complete, initialized from the procedural-SFT v0.13 checkpoint. On the same frozen 64-question test, semantic review gives vanilla/v0.13/v0.14 = 8/8/33; frozen automatic scores remain 0/5/30. The new student improves within this bounded domain but retains weaknesses and a 5/240 old-test regression. The original independent teacher gate remains failed; a separate new rule-scaffolded diagnostic passed unchanged thresholds before student training. Reserve evaluation questions from teacher-data generation and checkpoint selection. Do not change success criteria after seeing test outcomes.
