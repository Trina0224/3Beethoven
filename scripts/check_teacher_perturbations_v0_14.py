...
ROOT=Path('/kaggle/working/3beethoven_teacher_perturbations_v0_14')
...
def build():
    ...
    def add(group,a,b):
        i=len(pairs)
        for j,q in enumerate((a,b)):q['id']=f'teacher_check_{i:02d}_{j}'
    ...
    return dict(pairs=pairs,gate=dict(min_verified=44,min_pairs=20,min_group_fraction=.75),
                policy='48 independent prompts, one response each, no retries. Pending review does not pass. No test answers or references sent. Freeze before calls. No training on this diagnostic.')


def main():
    from kaggle_secrets import UserSecretsClient
    frozen=build();ROOT.mkdir(exist_ok=True)
    committed=json.loads((Path(__file__).resolve().parents[1]/'docs/STATS_V0_14_TEACHER_PERTURBATIONS.json').read_text())
    ...
    try:
        for pair in frozen['pairs']:
            for q in pair['questions']:
                ...
                raw=client.call(q['id'],[dict(role='system',content='Solve independently. Return a fully substituted numerical expression, not its evaluated final answer. Keep operations and conversion factors unevaluated. No explanation.'),dict(role='user',content=prompt(q))],max_tokens=200)
                rows.append(dict(pair_id=pair['id'],group=pair['group'],id=q['id'],raw=raw,judged=grade(raw,q)))
                save_json(ROOT/'responses.json',rows)
            print('PERTURBATION',len(rows),'/48',flush=True)
        ...
        print('PERTURBATION_RESULT',json.dumps(summary),flush=True)
