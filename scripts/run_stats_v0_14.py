"""v0.13 -> independently generated teacher response distillation. Run on Kaggle, not at import time."""
import gc
import hashlib
...
from stats_curriculum_v0_14 import build, digest, score
from prepare_verified_distillation_v0_14 import formulation_prompt as prompt, build as teacher_rows

ROOT=Path('/kaggle/working/3beethoven_stats_v0_14')
V13=Path('/kaggle/working/3beethoven_stats_v0_13')
SOURCE_SHA='7549a593f282e01b5b0b985dd20b5b2df0e52c390b0822ed637197327e2816ca'
...
def restore():
    target=V13/'adapter/adapter_model.safetensors'
    if not target.exists():
        import kagglehub
        saved=Path(kagglehub.notebook_output_download('trinashih/3beethoven-v0-2/versions/26'))
        candidates=sorted((p for p in saved.rglob(V13.name) if (p/'adapter/adapter_model.safetensors').exists()),key=lambda p:len(p.parts))
        if not candidates: raise RuntimeError('Saved v0.13 adapter not found; do not train from another checkpoint.')
        shutil.copytree(candidates[0],V13,dirs_exist_ok=True)
    if hashlib.sha256(target.read_bytes()).hexdigest()!=SOURCE_SHA:
        raise RuntimeError('v0.13 adapter hash mismatch')
...
def evaluate_formulations(model,tok,questions,path):
    ...
    for q in questions:
        ...
        save(path,rows)
        if len(rows)%12==0: print('V14 EVAL',path.stem,len(rows),'/',len(questions),flush=True)
    def counts(subset):
...
def main():
    ...
    protocol=dict(data_sha256=DATA_SHA,source_adapter_sha256=SOURCE_SHA,source='v0.13',
        ...
        test_role='Frozen v14 wording/parameter holdout; no teacher test targets; not general reasoning benchmark')
    ...
    assert {s:len(rs) for s,rs in rows.items()}=={'train':126,'validation':31}
    assert rows==read(repo/'docs/STATS_V0_14_VERIFIED_DISTILLATION.json')
    for s in rows: save(ROOT/f'{s}_examples.json',rows[s])
    token=UserSecretsClient().get_secret('HF_TOKEN')
    tok=AutoTokenizer.from_pretrained(V13/'adapter')
    ...
    if not training:
        model=PeftModel.from_pretrained(base(),V13/'adapter',is_trainable=True)
        ...
        args=TrainingArguments(output_dir=str(ROOT/'checkpoints'),num_train_epochs=2,
            ...
            report_to='none',remove_unused_columns=False,optim='paged_adamw_8bit',seed=1414)
        ...
        package(ROOT)
        print('V14 TRAINED',json.dumps(training),flush=True)
        del trainer,model;gc.collect();torch.cuda.empty_cache()
    results={}
    for name,adapter in (('baseline',None),('v13',V13/'adapter'),('v14',ROOT/'adapter')):
        model=base()
        ...
        print('V14 MODEL',name,json.dumps(results[name]),flush=True)
    ...
    print('V14 COMPLETE',json.dumps(summary),flush=True)
    print('V14 ARCHIVE',archive.stat().st_size,hashlib.sha256(archive.read_bytes()).hexdigest(),flush=True)
