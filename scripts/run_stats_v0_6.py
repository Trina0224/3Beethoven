...
from stats_curriculum_v0_5 import build as original_questions
from stats_holdout_v0_6 import build as new_questions
...
def main():
    ...
    protocol=dict(epochs=3,learning_rate=5e-5,seed=226,effective_batch=8,lora_r=16,lora_alpha=32,
        ...
        primary='fresh-story four-rotation accuracy and all-four-correct',
        caveat='Same question/sequence counts and optimizer schedule as v0.5, different instructional text and supervised token counts')
    ...
    shutil.copy2(Path(__file__).resolve().parent.parent/'docs'/'STATS_V0_6_PROTOCOL.md',ROOT/'protocol.md')
    restored=Path(kagglehub.notebook_output_download('trinashih/3beethoven-v0-2/versions/9'))
    source=restored/'3beethoven_stats_v0_5'
    adapter=source/'adapter'
    if not (adapter/'adapter_model.safetensors').exists():
        raise RuntimeError('Remove latest notebook input and mount version 9; never substitute another adapter')
    if hashlib.sha256((adapter/'adapter_model.safetensors').read_bytes()).hexdigest()!='c0c259efb287bf279537d62a6cb63d5df1c325a5ec89d295cf5c1054ee5e0c61':
        raise RuntimeError('v0.5 adapter mismatch')
    ...
    print('V06 TRAINING FROZEN',json.dumps(protocol),flush=True)
    ...
    for name in ('baseline','v05'):
        ...
        print('V06 MODEL',name,json.dumps({k:v['overall'] for k,v in summaries[name].items()}),flush=True)
    ...
    summaries['v06']={label:evaluate(model,tokenizer,qs,ROOT/f'v06_{label}.json') for label,qs in suites}
    ...
    print('V06 COMPLETE',json.dumps(summary),flush=True)
    print('V06 ARCHIVE',archive.stat().st_size,hashlib.sha256(archive.read_bytes()).hexdigest(),flush=True)
