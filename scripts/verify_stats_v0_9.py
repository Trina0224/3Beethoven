...
from generate_stats_v0_9 import ROOT
...
def main():
    ...
    total=0
    for name in ('baseline','v05','v09'):
        for suite in ('mc','numeric')+(('old',) if name=='v09' else ()):
    ...
    print('V09 VERIFIED',json.dumps(export['verification']),flush=True)
    print('V09 EXPORT',json.dumps(export),flush=True)
