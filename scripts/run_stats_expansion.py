"""Preregistered two-seed expansion response-distillation pilot.
...
def main():
    ...
    p.add_argument('--prior-v55',action='store_true')
    ...
    assert 708<=len(corpus['train'])<=772 and math.ceil(len(corpus['train'])/8)==decision['final_step']
    ...
    if a.prior_v55:
        assert a.mode=='probe' and a.seed is None
        adapter=a.input/'prior_v55_2027'
        assert sha(adapter/'adapter_model.safetensors')=='92eca7f1257880cb5c69bccd7bda8d600f0268bea62c9622d97551b1fbf6506a'
    ...
    if a.mode=='baseline':
        ...
        print('EXPANSION_BASELINE_COMPLETE',json.dumps(metrics),flush=True);return
    if a.mode=='probe':
        folder=a.root/'probe'/('prior_v55_2027' if a.prior_v55 else 'v15' if a.seed is None else str(a.seed))
        ...
        print('EXPANSION_PROBE_COMPLETE',json.dumps({'seed':a.seed,**suite_metrics(rows)}),flush=True);return
    ...
    class Validation(TrainerCallback):
        def on_step_end(self,args,state,control,model=None,**kwargs):
            if state.global_step in decision['validation_steps']:
                ...
                print('EXPANSION_CHECKPOINT',json.dumps({'seed':a.seed,'step':state.global_step,
    ...
    assert len(trace)==len(prepared)==len(rows)
    ...
    assert trainer.state.global_step==decision['final_step']
    save(output/'training_complete.json',{'global_steps':trainer.state.global_step,'train_loss':result.training_loss,
        'uninterrupted':True,'actual_microbatches_verified':len(trace),
    ...
    print('EXPANSION_TRAIN_COMPLETE',a.seed,flush=True)
