...
def call(root,key,tag,msgs):
    ...
    unresolved=set(starts)-{r['tag'] for r in done}
    assert unresolved <= {'expansion_train_events_03_neither_0','expansion_train_events_14_both_1'},unresolved
    assert tag not in unresolved,'Never retry an interrupted attempt'
    ...
    if len(starts)>=256 or sum(costs)+0.01*len(unresolved)+0.01>0.50:
    ...
    if response.status_code!=200:
        append(ledger,dict(event='http_error',tag=tag,status=response.status_code,retry_after=response.headers.get('Retry-After'),error_message=str(response.json().get('error',{}).get('message','')).replace(key,'[REDACTED]')[:500]))
...
def main():
    ...
    manifest['format_amendment_sha256']=digest(read_json(ROOT/'docs/STATS_EXPANSION_FORMAT_AMENDMENT.json'))
    manifest['rate_limit_continuation_sha256']=digest(read_json(ROOT/'docs/STATS_EXPANSION_RATE_LIMIT_CONTINUATION.json'))
    old=read_json(a.root/'rate_limit_continuation_manifest.json');assert old is None or old==manifest
    save_json(a.root/'rate_limit_continuation_manifest.json',manifest)
    ...
    for q in questions:
        for attempt in (0,1):
            ...
            if tag in {'expansion_train_events_03_neither_0','expansion_train_events_14_both_1'}:
                row=dict(id=q['id'],target=q['semantics']['target'],domain=q['semantics']['kind'],attempt=attempt,raw=None,accepted=False,classification='interrupted_unknown_billing_not_retried')
                attempts.append(row);save_json(a.root/'attempts.json',attempts)
                continue
            cached=read_json(a.root/'api_cache'/(tag+'.json'))
            if cached:
                msgs=cached['request']['messages']
            else:
                msgs[0]['content']+=' CRITICAL FORMAT: Use INTEGER literals and fractions only in EVERY field, including intermediates. Never use decimal literals. Convert percentages with division by 100. Return exactly one valid JSON object with balanced braces and no extra closing brace.'
            record=call(a.root,key,tag,msgs)
        ...
        usage=dict(unknown_billing_reserved_usd=0.02,interrupted_attempts=1,http_failed_attempts=1,calls=sum(r['event']=='started' for r in ledger),cost=sum(r['usage']['cost'] for r in ledger if r['event']=='response'))
