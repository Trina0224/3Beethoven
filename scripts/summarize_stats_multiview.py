"""Review collection and frozen post-training selection for the multiview pilot."""
...
def main():
    ...
    questions.update({q['id']:q for q in read(REPO/'docs/STATS_MULTIVIEW_CORPUS.json')['transfer_probe']})
    if a.review_file:
    ...
    if a.select:
        ...
        for seed,item in summary['seeds'].items():
            ...
            selection={'selected_step':passing['step'] if passing else None,
                'fixed_step':passing['step'] if passing else 65,'all_pending_resolved':True,
    ...
    probe_questions=read(REPO/'docs/STATS_MULTIVIEW_CORPUS.json')['transfer_probe']
    lookup={q['id']:q for q in probe_questions}
    for path in (root/'probe').glob('*/rows.json'):
        rows=read(path);cells={}
        for numbers in ('known','new'):
            for wording in ('familiar','unseen'):
                group=[r for r in rows if lookup[r['id']]['numbers']==numbers and lookup[r['id']]['wording']==wording]
                cells[numbers+'_'+wording]=suite_metrics(group)
        summary['probe'][path.parent.name]={'n':len(rows),'cells':cells,'overall':suite_metrics(rows)}
    save(root/'MULTIVIEW_SUMMARY.json',summary)
    print('MULTIVIEW_SUMMARY',json.dumps(summary),flush=True)
