...
def call(root,key,tag,msgs):
    ...
    assert unresolved <= {'expansion_train_events_03_neither_0'},unresolved
...
def main():
    ...
    old=read_json(a.root/'format_continuation_manifest.json');assert old is None or old==manifest
    save_json(a.root/'format_continuation_manifest.json',manifest)
    ...
    for q in questions:
        for attempt in (0,1):
            ...
            if tag=='expansion_train_events_03_neither_0':
        ...
        usage=dict(unknown_billing_reserved_usd=0.01,interrupted_attempts=1,calls=sum(r['event']=='started' for r in ledger),cost=sum(r['usage']['cost'] for r in ledger if r['event']=='response'))
